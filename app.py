...
DB_URI = os.getenv("DB_URI") or os.getenv("DATABASE_URL") or st.secrets.get("DATABASE_URL") or st.secrets.get("DB_URI")
...
if not DB_URI or not OPENAI_API_KEY:
    st.error(
        "Missing configuration! Please check that DB_URI/DATABASE_URL and OPENAI_API_KEY are"
...
def init_vector_store():
    ...
    vector_store = PGVector(
        ...
        connection=DB_URI,
        use_jsonb=True,
        create_extension=False,  # Prevent automated extension creation crashes on AWS RDS
    )
    return vector_store
